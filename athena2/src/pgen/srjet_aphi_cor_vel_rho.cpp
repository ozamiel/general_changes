...
void JetInnerX3(MeshBlock *pmb, Coordinates *pco, AthenaArray<Real> &prim, FaceField &b,
 ...
 //std::cout << "kl = " << kl << "\n";
	...
	prim(IVZ,kl-k,j,i) = sqrt((gamma*gamma - 1.)/(1. + SQR(rang(r_0))*std::exp((-2.*z/z_0)) + phang*phang));
   ...
   for (int k=1; k<=ngh; ++k) {
    for (int i=il; i<=iu; ++i) {
       ...
       for (int j=jl; j<=ju; ++j){
           if (r<=r_jet + dr_jet && r>x1min){
               //std::cout << k << " " << i << " " << j << "\n";
           prim(IVX,kl-k,j,i) = prim(IVZ,kl-k,j,i)*bcc(IB1,kl-k,j,i)/(bcc(IB3,kl-k,j,i));
               //std::cout << "hello\n";
           }
