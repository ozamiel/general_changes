...
void JetInnerX3(MeshBlock *pmb, Coordinates *pco, AthenaArray<Real> &prim, FaceField &b,
  ...
  for (int k=1; k<=ngh; ++k) {
	...
	if(MAGNETIC_FIELDS_ENABLED) {
	  ...
	  Real Bz = b_0 * (1./(1.+R*R/(a*a))) * smfnc; //Setting Bz just for calculation purposes - athena does not "know" about it.
	  b.x2f(kl-k,j,i) = (b_0*a*R/(a*a+R*R)) * smfnc; //Setting Bphi = R/a * Bz - force-free solution (total pressure-free in our case).
	  
	  //b.x1f(kl-k,j,i) = 0.0;
	  //b.x3f(kl-k,j,i) = 0.0;
	  prim(IPR,kl-k,j,i) = p_amb + Bz*Bz/(2.); //Setting pressure so that total pressure is constant and is p_amb for pressure balance.
	  ...
	  Real b_z_cen = b_0 * (1./(1.+x1min*x1min/(a*a))) * smfnc_c;
	  Real b_phi_cen = (b_0*a*x1min/(a*a+x1min*x1min)) * smfnc_c;
	  Real p_cen = p_amb + b_z_cen*b_z_cen/2.; //Setting this because prim(IPR,kl-k,0,0) would bring not the pressure at the center but at the beginning of the domain
	  Real bern_jet = (1. + (gam_add*p_cen/d_jet))*gamma_jet + b_phi_cen*b_phi_cen/(gamma_jet*d_jet); //Setting the Bernoulli parameter and smoothing it
	  ...
	  Real gamma = (Psi/(2.*gam_add*press)) * (sqrt(1. + (4.*gam_add*press*atwd_sm)/(Psi*Psi)) - 1.);
      ...
      // (vy_jet*rad-vy_amb) * step + vy_amb; 
	    //Real x1_m = pco->x1f(i);
	    //Real x1_p = pco->x1f(i+1);
	    //Real del_x1 = x1_p - x1_m;
	    //Real del_A3 = integ_A3(x1_m,x1_p);
	    //if(del_x1 > 0.0) {
	    // b.x2f(kl-k,j,i) = -(del_A3)/(del_x1);
