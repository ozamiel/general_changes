...
void JetInnerX3(MeshBlock *pmb, Coordinates *pco, AthenaArray<Real> &prim, FaceField &b,
  ...
  for (int k=1; k<=ngh; ++k) {
	...
	if(MAGNETIC_FIELDS_ENABLED) {
	  ...
	  // Real p_0 = 1.*b_0;
	  if(R<a) {
	    b.x2f(kl-k,j,i) = (b_0*R/a) * smfnc; //Setting the magnetic field
	  } else {
	    b.x2f(kl-k,j,i) = (b_0*a/R) * smfnc;
	  }

	  //b.x1f(kl-k,j,i) = 0.0;
	  //b.x3f(kl-k,j,i) = 0.0;
	  prim(IPR,kl-k,j,i) = p_amb - b.x2f(kl-k,j,i)*b.x2f(kl-k,j,i)/(2.); //Setting pressure so that total pressure is constant and is p_amb for pressure balance.
